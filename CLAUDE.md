...
- **`rex_config_form::addCheckboxField` stores ticked checkboxes as `'|<opt>|'`** (pipe-delimited because checkboxes can hold multiple options — `|1|2|` for two ticked options with values `1` and `2`). A naive `(bool) (int) $value` cast on `'|1|'` evaluates to `false` because PHP int-casts strings that don't start with a digit to `0`. Symptom: a user ticks the checkbox in the settings page, the value persists to `rex_config`, but the addon behaves as if it's still off. Worse, `Config::get` falls back to `DEFAULTS` whenever the saved value is `''`, so an unticked checkbox _also_ silently reads as the default — making it impossible to disable a default-on setting from the UI. Use `Config::checkboxBool($key)` for any checkbox-backed bool: it goes directly to `rex_config::get` (bypassing the empty-string fallback), strips pipes, and then int-casts. Tests in `tests/Unit/ConfigTest.php` lock in the `'|1|'`, `''`, and integer-default code paths. Apply this pattern to every new checkbox config you add — never `(bool) (int) self::get(...)`.
